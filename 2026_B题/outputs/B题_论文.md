...
实际求解时先在允许区间内粗搜索，再用有界一维优化细化 $\delta$。对问题2、问题3，本文比较无偏模型和带偏模型的残差平方和，采用嵌套模型F检验判断固定偏差项是否具有统计显著性（$\alpha=0.05$）：若p值小于0.05，则判定存在系统偏差，使用带偏模型估计偏差值并修正轨迹。考虑到实际测量数据样本量较大时微小均值漂移也可能被检出，本文在结果中附加工程效应量（误差下降比例和偏差模长）作为量级参考，但不改变统计检验的确定性结论。
...
| 3 | -367.9164 | [-367.9237,-367.9086] | -0.1495 | -0.2095 | -0.1495 | -0.2095 | 1.65% | 存在并修正（量级较小） |
